...
CREATE INDEX idx_overseer_audit_timestamp ON overseer_audit_log(timestamp DESC);

-- Index for queries by action
CREATE INDEX idx_overseer_audit_action ON overseer_audit_log(action);

-- Index for queries by user
CREATE INDEX idx_overseer_audit_user ON overseer_audit_log(user_id) WHERE user_id IS NOT NULL;
